...
const ALL_TECHS = [
  // SLIDE 1 ITEMS ON DESKTOP
  { name: "Next.js",    icon: SiNextdotjs,   color: "#111111", bg: "rgba(17, 17, 17, 0.04)", border: "rgba(17, 17, 17, 0.15)",   tag: "Framework", span: "col-span-1 sm:col-span-2 md:col-span-2", featured: true },
  ...
  { name: "Unity",      icon: SiUnity,       color: "#000000", bg: "rgba(0, 0, 0, 0.04)",       border: "rgba(0, 0, 0, 0.15)",        tag: "Engine",    span: "col-span-1 sm:col-span-2 md:col-span-2", featured: true },
  ...
  // SLIDE 2 ITEMS ON DESKTOP
  { name: "Git",        icon: SiGit,         color: "#F05032", bg: "rgba(240, 80, 50, 0.04)",   border: "rgba(240, 80, 50, 0.15)",    tag: "Version",  span: "col-span-1 sm:col-span-2 md:col-span-2", featured: true },
...
export default function TechStack() {
  ...
  // Pre-calculate slides for both device views
  const desktopSlides = chunkTechs(ALL_TECHS, 7);
  const mobileSlides = chunkTechs(ALL_TECHS, 3);

  // Reusable Tech Card Component to keep things clean and DRY
  const TechCard = ({ tech, globalIndex }: { tech: typeof ALL_TECHS[0]; globalIndex: number }) => (
    <motion.div
      className={`group relative flex flex-col justify-between p-5 sm:p-6 rounded-2xl overflow-hidden cursor-pointer ${tech.span}`}
      style={{
    ...
    >
      ...
      <div className="w-full flex justify-between items-center z-10">
        ...
        <span
          className="font-mono text-[10px] sm:text-xs tracking-wider uppercase px-2 py-0.5 rounded-md font-medium whitespace-nowrap"
          style={{
      ...
      <div className="flex items-center gap-2 sm:gap-3 z-10 mt-auto pt-2">
        <tech.icon
          size={tech.featured ? 24 : 20}
          style={{ color: tech.color }}
          className="drop-shadow-[0_2px_4px_rgba(0,0,0,0.02)] transition-transform duration-300 group-hover:scale-105 shrink-0"
        />
        <span
          className={`font-mono tracking-wide uppercase text-[var(--primary)] truncate ${
            tech.featured ? "text-sm sm:text-base font-bold" : "text-xs font-medium"
          }`}
        >
          {tech.name}
        </span>
  ...
  return (
    <section
      id="tech-stack"
      className="relative w-full lg:min-h-screen bg-[var(--background)] flex items-center px-6 md:px-12 lg:px-16 py-16 lg:py-20 overflow-hidden"
    >
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 items-start selection:bg-[var(--primary)] selection:text-[var(--background)]">
        ...
        >
          {/* ────────────────── DESKTOP & TABLET VIEW (7 items/slide, asymmetric design) ────────────────── */}
          <div className="hidden md:block w-full">
            ...
            >
              {desktopSlides.map((slideItems, slideIndex) => (
                <SwiperSlide key={`desktop-slide-${slideIndex}`} className="w-full">
                  <div className="w-full grid grid-cols-3 gap-4 auto-rows-[150px] p-1 pb-12">
                    {slideItems.map((tech, i) => {
                      const globalIndex = slideIndex * 7 + i + 1;
                      return <TechCard key={`desktop-${tech.name}`} tech={tech} globalIndex={globalIndex} />;
                    })}
                  </div>
                </SwiperSlide>
              ))}
          ...
          {/* ────────────────── MOBILE VIEW (3 items/slide, perfectly stacked clean list) ────────────────── */}
          <div className="block md:hidden w-full">
            ...
            >
              {mobileSlides.map((slideItems, slideIndex) => (
                <SwiperSlide key={`mobile-slide-${slideIndex}`} className="w-full">
                  <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-3 auto-rows-[130px] sm:auto-rows-[140px] p-1 pb-12">
                    {slideItems.map((tech, i) => {
                      const globalIndex = slideIndex * 3 + i + 1;
                      return <TechCard key={`mobile-${tech.name}`} tech={tech} globalIndex={globalIndex} />;
                    })}
                  </div>
                </SwiperSlide>
              ))}
